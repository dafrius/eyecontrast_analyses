...
load(paste(path,"/scripts/ec_pilot.rda",sep=""))
load(paste(path,"/scripts/fit_ec_pilot.rda", sep=""))
load(paste(path,"/scripts/preds_fit_ec_pilot.rda", sep=""))
```
...
subinfo3 <- cbind(subinfo, subinfo2$dec..VA, subinfo2$logMAR, subinfo2$Handiness, subinfo2$Fac_recog)
...
simple_d_lohi <- df_trns %>% group_by(subject, task, congruency, contrast3) %>%
  summarise(n=n(), mean=mean(acc))

d_binned <- simple_d_lohi %>% ungroup() %>% group_by(task) %>% filter(task != "low") %>%
  # I bin everything to 4 levels
  mutate(contbin = ntile(contrast3, n=4)) %>% group_by(task, congruency, contbin) %>% 
  ...
  mutate(task= case_when(task=="up" ~ "Upright Face",
         ...
         congruency = case_when( congruency == "cong" ~ "Congruent",
                                 congruency == "inc" ~ "Incongruent",
...
d_binned$congruency <- ordered(d_binned$congruency, levels = c("Incongruent", "Congruent","Isolated"))

d_binned %>% ggplot(aes(x=contrast, y=mean, color=congruency))+
  geom_point(aes(size=2.5))+
  geom_line(aes(group=congruency))+
...
d_distances <- d_binned %>% select(task, congruency, contbin, contrast, mean) %>% pivot_wider(names_from=c(congruency), values_from=c(contrast, mean)) %>%
  mutate(dist= sqrt((contrast_Congruent - contrast_Incongruent)^2 + (mean_Congruent - mean_Incongruent)^2)) 
...
tail(sort(rstan::summary(fit_ec_pilot$fit)$summary[,"Rhat"]))
head(sort(rstan::summary(fit_ec_pilot$fit)$summary[,"n_eff"]))
pars <- variables(fit_ec_pilot)
pars_sel <- c(sample(pars[1:10], 3), sample(pars[-(1:10)], 3))
plot(fit_ec_pilot, variable = pars_sel, N = 6, 
...
d_binned <- simple_d_lohi %>% ungroup() %>% group_by(congruency, task, subject) %>%
  # I bin everything to 5 levels so that the comparison across the data and the model is simpler
  mutate(contbin = ntile(contrast3, n=5)) %>% group_by(subject, task, congruency, contbin) %>% 
...
for(i in unique(df_trns$subject)){
  for(j in unique(df_trns$task)){
      for(t in unique(df_trns$congruency)){
        try <- df_trns %>% 
                  filter(subject==i, task==j, congruency==t) %>% 
                  group_by(contrast3) %>%
...
sumcoefs <- coef(fit_ec_pilot)$subject
...
sumco2 <- sumco1 %>% select(subject, starts_with("Esti")) %>% # Only getting the "Estimate" values
  ...
  mutate(int_inv_cong = Intercept,
         slp_inv_cong = contrast3,
         int_low_cong = Intercept + tasklow,
         slp_low_cong = contrast3 + `contrast3:tasklow`,
         int_up_cong = Intercept + taskup,
         slp_up_cong = contrast3 + `contrast3:taskup`,
         int_inv_inc = Intercept + congruencyinc,
         slp_inv_inc = contrast3 + `contrast3:congruencyinc`,
         int_low_inc = Intercept + `tasklow` + `congruencyinc` + `tasklow:congruencyinc`,
         slp_low_inc = contrast3 + `contrast3:tasklow` + `contrast3:congruencyinc` + `contrast3:tasklow:congruencyinc`,
         int_up_inc = Intercept + `taskup` + `congruencyinc` + `taskup:congruencyinc`,
         slp_up_inc = contrast3 + `contrast3:taskup` + `contrast3:congruencyinc` + `contrast3:taskup:congruencyinc`,
         int_inv_iso = Intercept + congruencyiso,
         slp_inv_iso = contrast3 + `contrast3:congruencyiso`,
         int_low_iso = Intercept + tasklow + congruencyiso + `tasklow:congruencyiso`,
         slp_low_iso = contrast3 + `contrast3:tasklow` + `contrast3:congruencyiso` + `contrast3:tasklow:congruencyiso`,
         int_up_iso = Intercept + `taskup` + `congruencyiso` + `taskup:congruencyiso`,
         slp_up_iso = contrast3 + `contrast3:taskup` + `contrast3:congruencyiso` + `contrast3:taskup:congruencyiso`) %>% #calculating the actual slope and intercept values
  select(subject, starts_with("int_"), starts_with("slp")) %>%
  pivot_longer(2:19, names_to="param", values_to="val") %>%
  separate(param, into=c("param","task","congruency"), sep="_") %>%
...
etas <- sumco2 %>% mutate(task=case_when(task=="inv"~"Inverted Face",
                           ...
                           congruency = case_when(congruency=="iso"~"Isolated",
                                               congruency=="cong"~"Congruent",
                                               congruency=="inc"~"Incongruent"))
save(etas, file="ec_pilot_etas.rda")
...
load(paste(path,"/scripts/ec_pilot_etas.rda",sep=""))
load(paste(path,"/scripts/preds_fit_ec_pilot.rda", sep=""))
...
etaslow %>% group_by(congruency) %>% summarise(locs=mean(intercept))

etaslow %>% ggplot(aes(x=congruency, y=location))+
  geom_boxplot()
...
for(i in unique(pred_df$subject)){
  ...
  d_av<- df_trns %>% group_by(contrast3, congruency, task, subject) %>% 
    summarise(mean_p = mean(acc))

  d_count <- df_trns %>% group_by(contrast3, congruency, task, subject) %>% 
    summarise(n=n(), mean=mean(acc))

  tasks3n <- d_count %>% filter(subject==filter_subject)

  tasks3n2 <- tasks3n  %>% ungroup() %>% group_by(congruency,task) %>%
    mutate(contrastbin = ntile(contrast3, n=5)) %>% group_by(congruency, task, contrastbin) %>% 
    ...
    select(contrast3, congruency, task, subject, n, mean)
  ...
  pred_df2$congruency <- ordered(pred_df2$congruency, levels = c("Incongruent", "Congruent","Isolated"))
...
fig <- pred_df2 %>% ggplot(aes(x = contrast3, y = Estimate, color=congruency)) + 
    geom_line() + 
    geom_ribbon(aes(ymin = Q2.5, ymax = Q97.5, fill=congruency), alpha = .4, color="NA") + 
    ...
    geom_vline(data=tasks3ls, aes(xintercept=location, color=congruency))+
...
b1 <- etas %>% select(location, subject, task, congruency) %>%
  pivot_wider(names_from="congruency", values_from="location") %>%
  pivot_longer(3:5, names_to="congruency", values_to="val") %>%
  pivot_wider(names_from=c("task","congruency"), values_from="val")

b2 <- b1 %>% pivot_longer(2:10, names_to="name",values_to="val")
b3 <- b2 %>% separate(name, into=c("task","congruency"), sep="_") %>%
  mutate(jit1 = case_when(congruency=="Congruent"~2,
                          congruency=="Isolated"~3,
                          congruency=="Incongruent"~1),
         jit2 = jitter(jit1, factor=1.8))


threshmean <- b3 %>% filter(val < 30, val >-10) %>% group_by(task, congruency) %>%
  summarise(val=mean(val))

threshmean$task <- ordered(threshmean$task, levels = c("Upright Face", "Inverted Face","Contrast Detection"))
threshmean$congruency <- ordered(threshmean$congruency, levels = c("Incongruent", "Congruent","Isolated"))
threshmean <- arrange(threshmean, congruency)

b3$task <- ordered(b3$task, levels = c("Upright Face", "Inverted Face","Contrast Detection"))
b3$congruency <- ordered(b3$congruency, levels = c("Incongruent", "Congruent","Isolated"))


b3 %>% filter(val <= 20, val >-10) %>% ggplot(aes(x=jit2, y=val, color=congruency))+
  geom_boxplot(aes(x=congruency,fill=congruency), alpha=.5, outlier.shape=NA, notchwidth=0)+
  geom_point(aes(group=subject),size=2.5)+
  geom_line(aes(group=subject), color="gray", alpha=.5)+
  facet_wrap(~task)+
  stat_boxplot(aes(x=congruency,group=congruency),geom="errorbar", width=.1)+
  theme_classic()+
  ggthemes::scale_fill_colorblind() + 
  ggthemes::scale_color_colorblind() +
  ylab("Threshold")+
  xlab("congruency")+
  geom_point(data=threshmean, aes(x=congruency, y=val), color="black", alpha=.6, size=5)+
  ...
  geom_path(data=threshmean, aes(x=congruency, y=val, group=task), color="black", alpha=.6,linewidth=1.5)
...
load(paste(path,"/modeling/model_data/ec_pilot3_etas.rda",sep=""))
...
b1 <- etas3 %>% select(location, subject, task, congruency) %>%
  mutate(task=case_when(task=="Inverted Face" ~ "inv",
                         task=="Upright Face" ~ "up",
                         task=="Contrast Detection" ~ "gra"),
         congruency=case_when(congruency=="Congruent" ~ "cong",
                              congruency=="Incongruent" ~ "inc",
                              congruency=="Isolated" ~ "iso")) %>%
  pivot_wider(names_from="congruency", values_from="location") %>%
  mutate(C_I = cong-inc) %>% pivot_longer(3:6, names_to="congruency", values_to="val") %>%
  pivot_wider(names_from=c("task","congruency"), values_from="val") 

b2 <- b1

# Upright - Iso/inc
uiinc <- lm(up_inc~up_iso, data=b2)
b2$uiincpredicted <- predict(uiinc)
b2$uiincresid <- residuals(uiinc)


# Upright - Iso/cong
uic <- lm(up_cong~up_iso, data=b2)
b2$uicpredicted <- predict(uic)
b2$uicresid <- residuals(uic)

# Inv - Iso/inc
iiinc <- lm(inv_inc~inv_iso, data=b2)
b2$iiincpredicted <- predict(iiinc)
b2$iiincresid <- residuals(iiinc)

# Inv - Iso/cong
iic <- lm(inv_cong~inv_iso, data=b2)
b2$iicpredicted <- predict(iic)
b2$iicresid <- residuals(iic)

# Low - Iso/inc
giinc <- lm(gra_inc~gra_iso, data=b2)
b2$giincpredicted <- predict(giinc)
b2$giincresid <- residuals(giinc)

# Low - Iso/cong
gic <- lm(gra_cong~gra_iso, data=b2)
b2$gicpredicted <- predict(gic)
b2$gicresid <- residuals(gic)

b3 <- b2 %>% select(subject, uiincresid, uicresid, iiincresid, iicresid, giincresid, gicresid)

# Up - inc ~ cong
uincc <- lm(uiincresid~uicresid, data=b3)
b3$uinccpredicted <- predict(uincc)
b3$uinccresid <- residuals(uincc)

# Inv - inc ~ cong
iincc <- lm(iiincresid~iicresid, data=b3)
b3$iinccpredicted <- predict(iincc)
b3$iinccresid <- residuals(iincc)

# Low - inc ~ cong
gincc <- lm(giincresid~gicresid, data=b3)
b3$ginccpredicted <- predict(gincc)
b3$ginccresid <- residuals(gincc)


# Up - cong ~ inc 
ucinc <- lm(uicresid~uiincresid, data=b3)
b3$ucincpredicted <- predict(ucinc)
b3$ucincresid <- residuals(ucinc)

# Inv - cong ~ inc
icinc <- lm(iicresid~iiincresid, data=b3)
b3$icincpredicted <- predict(icinc)
b3$icincresid <- residuals(icinc)

# Low - cong ~ inc 
gcinc <- lm(gicresid~giincresid, data=b3)
b3$gcincpredicted <- predict(gcinc)
b3$gcincresid <- residuals(gcinc)


b4 <- b3 %>% select(subject, uinccresid, iinccresid, ginccresid, ucincresid, icincresid, gcincresid)
...
cor.test(b4$udsresid, b4$ldsresid)
